...
COQLIBS?= -R ../../Sophia-Antipolis/Algebra/ Algebra\
  -R . LinAlg
COQDOCLIBS?=-R ../../Sophia-Antipolis/Algebra/ Algebra\
  -R . LinAlg
...
VFILES:=extras/Equality_structures.v\
...
install:
	cd "." && for i in $(VOFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/LinAlg/$$i`"; \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/LinAlg/$$i; \
	done
...
uninstall_me.sh:
	echo '#!/bin/sh' > $@ 
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/LinAlg && rm -f $(VOFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "LinAlg" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
